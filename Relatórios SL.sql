...
select dep.nome "Departamento",
	concat('R$ ', format(avg(salario),2,'de_DE')) "Média Salárial",
...
select emp.nome "Nome", 
	...
	emp.sexo "Sexo",
	concat("R$ ", format(emp.salario, 2, 'de_DE')) "Salário",  
	concat("R$ ", format(sum(serv.valorVenda -  coalesce(isv.desconto, 0)), 2, "de_DE")) "Valor Total de venda com Serviço",
...
emp.nome "Empregado que fez a venda"
	from venda vend
...
select
    fpv.tipo "Tipo de forma de Pagamento",
    count(fpv.Venda_idVenda) "Quantidade_Vendas",
    concat("R$ ", replace(format(sum(vend.valor), 2), '.', ',')) "Total do valor vendido"
